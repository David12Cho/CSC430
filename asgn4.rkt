  ...
  (list
   (Bind '+ (PrimOpV (lambda (args)
                      (if (= (length args) 2)
                          ...
                          (error "ZODE: + requires exactly two arguments."))) 
                   2))
   (Bind '- (PrimOpV (lambda (args)
                      (if (= (length args) 2)
                          (let ([arg1 (car args)] [arg2 (cadr args)])
       ...
       (match f
         [(CloV params body closure-env)
          (if (= (length params) (length args))
              ...
              (error 'interp "ZODE: Incorrect number of arguments for function: expected ~a, got ~a" (length params) (length args)))]
         [(PrimOpV proc arity)
          (if (= arity (length args))
              ...
              (let ([evaluated-args : (Listof Value) (evaluate-arguments args env)])
                (proc evaluated-args))
              (error 'interp "ZODE: Incorrect number of arguments for primop: expected ~a, got ~a" arity (length args)))]
         [else
