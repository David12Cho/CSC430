...
(define-type ExprC (U NumC IdC StrC IfC LambC AppC SeqC))
...
; Assuming serialize handles value to string conversion
(define (concatenate-strings [args : (Listof Value)]) : Value
  (string-join (map serialize args) ""))  ; Joins all strings without a delimiter

; Function to evaluate a sequence of expressions and return the last result
(define (seq [args : (Listof Value)]) : Value
  (define (rec-seq [lst : (Listof Value)] [last-val : Value]) : Value 
    (if (null? lst)
        last-val
        (rec-seq (cdr lst) (car lst))))
  (if (null? args)
      (error "ZODE: seq: No expressions provided")
      (rec-seq (cdr args) (car args))))

; Add 'seq' to the environment with type annotations and variable arity
(define (extend-with-seq [env : Env]) : Env
  (cons (Bind 'seq (PrimOpV (lambda ([args : (Listof Value)]) : Value
                             (seq args))
                          -1)) ; Using -1 or another special value to indicate variable arity
        env))

(define (concatenate [args : (Listof Value)]) : String
...
; Assuming we allow variable arity, we might use a special marker or a negative value to indicate this
(define (extend-with-concatenate [env : Env]) : Env
  (cons (Bind '++ (PrimOpV (lambda ([args : (Listof Value)]) : Value
                             (concatenate args))
                          -1)) ; -1 or some other special value to denote variable arity
        env))
...
;;(struct PrimOpV ([proc : ((Listof Value) -> Value)]) #:transparent)
...
(define top-env
  (list
   ...
   (Bind 'equal? (PrimOpV (lambda (args)
                          ...
                          2))
   (Bind '++ (PrimOpV (lambda (args)
                    (apply string-append (map serialize args))) 
                   -1))  ; -1 arity to indicate variable number of arguments
   ...
   (Bind 'read-num (PrimOpV (λ (args)
                            ...
                            0))
   (Bind '++ (PrimOpV concatenate-strings -1)) ; -1 or use a more dynamic check for variable arity handling
...
; Assuming a direct environment and seq function available
(check-equal? (seq (list 1 2 3)) 3 "seq should return the last number")
(check-equal? (seq (list "hello" " " "world")) "world" "seq should return the last string")
(check-equal? (seq (list true false true)) true "seq should return the last boolean") 
 
; Handling edge case with an empty list
(check-exn exn:fail? (lambda () (seq '())) "seq should fail on empty list")
